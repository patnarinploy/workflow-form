...
export async function GET(req: Request) {
  ...
  if (!getStaff(person)) return NextResponse.json({ error: "ไม่พบพนักงานคนนี้" }, { status: 400 });
...
export async function POST(req: Request) {
  ...
  if (!body.personId || !getStaff(body.personId)) return NextResponse.json({ error: "ไม่พบพนักงานคนนี้" }, { status: 400 });
...
export async function DELETE(req: Request) {
  ...
  if (!getStaff(person) || !project) return NextResponse.json({ error: "ข้อมูลไม่ครบ" }, { status: 400 });
  const supabase = createServiceClient();
